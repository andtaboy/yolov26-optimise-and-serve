"""Exercise 4a — pose FP16 via Triton HTTP (:8000)."""
...
def main():
    parser = argparse.ArgumentParser(description="Triton HTTP pose client (Exercise 4a).")
    ...
    parser.add_argument(
        "--save-overlay",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "pose_http.jpg",
    )
    parser.add_argument(
        "--json-out",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "pose_http.json",
    )
    args = parser.parse_args()

    if not args.skip_metadata:
        base = f"http://{args.url}"
        fetch_kserve(f"{base}/v2/health/ready")
        fetch_kserve(f"{base}/v2/models/{MODEL_NAME}")
        fetch_kserve(f"{base}/v2/models/{MODEL_NAME}/config")
    ...
    if not client.is_server_ready():
        raise RuntimeError(f"Triton HTTP not ready at {args.url}")
    ...
    print_metrics(f"Triton HTTP pose ({MODEL_NAME}):", metrics)
    write_metrics_json(
        args.json_out,
        metrics,
        extra={
